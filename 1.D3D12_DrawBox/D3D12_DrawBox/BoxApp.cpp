...
private:
	...
	std::unique_ptr<MeshGeometry> mBoxColor = nullptr;
...
void BoxApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->IASetVertexBuffers(1, 1, &mBoxColor->VertexBufferView());
...
void BoxApp::BuildBoxGeometry()
{
	...
	const UINT posvbByteSize = (UINT)PosVertices.size() * sizeof(VPosData);
...
void BoxApp::BuildBoxColor() 
{
	...
	const UINT colorvbByteSize = (UINT)ColorVertices.size() * sizeof(VColorData);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);

	mBoxColor = std::make_unique<MeshGeometry>();
	mBoxColor->Name = "box_Color";


	ThrowIfFailed(D3DCreateBlob(colorvbByteSize, &mBoxColor->VertexBufferCPU));
	CopyMemory(mBoxColor->VertexBufferCPU->GetBufferPointer(), ColorVertices.data(), colorvbByteSize);

	ThrowIfFailed(D3DCreateBlob(ibByteSize, &mBoxColor->IndexBufferCPU));
	CopyMemory(mBoxColor->IndexBufferCPU->GetBufferPointer(), indices.data(), ibByteSize);

	mBoxColor->VertexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), ColorVertices.data(), colorvbByteSize, mBoxColor->VertexBufferUploader);

	mBoxColor->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), indices.data(), ibByteSize, mBoxColor->IndexBufferUploader);

	mBoxColor->VertexByteStride = sizeof(VColorData);
	mBoxColor->VertexBufferByteSize = colorvbByteSize;
	mBoxColor->IndexFormat = DXGI_FORMAT_R16_UINT;
	mBoxColor->IndexBufferByteSize = ibByteSize;
	...
	mBoxColor->DrawArgs["cbox"] = submesh;
}
...
void BoxApp::BuildPyramidGeometry() {
	...
	{
		pyramidVertex({ XMFLOAT3(-1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) }),
		pyramidVertex({ XMFLOAT3(+1.0f, -1.0f, -1.0f), XMFLOAT4(Colors::Green) }),
		pyramidVertex({ XMFLOAT3(-1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),
		pyramidVertex({ XMFLOAT3(+1.0f, -1.0f, +1.0f), XMFLOAT4(Colors::Green) }),
		pyramidVertex({ XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT4(Colors::Red) })
	};
	...
	const UINT vbByteSize = (UINT)vertices.size() * sizeof(pyramidVertex);
	const UINT ibByteSize = (UINT)indices.size() * sizeof(std::uint16_t);
	...
	mPyramidGeo->IndexBufferGPU = d3dUtil::CreateDefaultBuffer(md3dDevice.Get(),
		mCommandList.Get(), vertices.data(), vbByteSize, mPyramidGeo->IndexBufferUploader);

	mPyramidGeo->VertexByteStride = sizeof(pyramidVertex);
